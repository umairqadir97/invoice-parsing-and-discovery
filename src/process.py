...
import os
import time
from src.AbbyyOnlineSdk import *
...
def recognize_file(file_path, result_file_path, language, output_format):
	...
	if task is None:
		print("Error processing documents with OCR")
		return False, "Error processing documents with OCR"
	if task.Status == "NotEnoughCredits":
		print("Not enough credits to process the document. Please add more pages to your application's account.")
		return False, "Not enough credits to process the document"
	...
	if task.Status == "Completed":
		if task.DownloadUrl is not None:
			...
			return True, "Successfully written to {}".format(result_file_path)
	else:
		print("Error processing documents with OCR")
		return False, "Error processing documents with OCR"


def main(source_file, target_file, language='English', output_format='txt'):
	global processor
	processor = AbbyyOnlineSdk()
	setup_processor()

	if not os.path.isfile(source_file):
		return False, "No such file: {}".format(source_file)
	status, message = recognize_file(source_file, target_file, language, output_format)
